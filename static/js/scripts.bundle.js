                ...
                if (typeof jQuery !== 'undefined' && $.fn.daterangepicker) {
                    ...
                    elements.map(function (element) {
                        if (element.getAttribute("data-kt-initialized") !== "1") {
                            ...
                            var opens = element.hasAttribute("data-kt-daterangepicker-opens") ? element.getAttribute("data-kt-daterangepicker-opens") : "left";
            
                            function updateData(start, end) {
                                // Send a request to the Flask backend to get filtered data
                                $.post('/get_filtered_data', {
                                    start_date: start.format('YYYY-MM-DD'),
                                    end_date: end.format('YYYY-MM-DD')
                                }, function (response) {
                                    // Handle the response and update the table or perform other actions
                                ...
                                if (container) {
                                    container.innerHTML = start.format("D MMM YYYY") + " - " + end.format("D MMM YYYY");
                                }
                            }
                            ...
                            function updateTable(data) {
                                ...
                                data.forEach(function (row) {
                                    var newRow = '<tr>' +
                                        ...
                                        '<img src="data:image/jpeg;base64, ' + row[1] + '">' +
                                        '</div>' +
                                        '<div class="d-flex justify-content-end flex-column">' +
                                        '<span class="text-gray-800 fw-semibold d-block fs-7">' + row[2] + '</span>' +
                                        '</div>' +
                                        '</div>' +
                                        '</td>' +
                                        '<td>' +
                                        '<div class="text-center pe-0">' +
                                        '<span class="text-gray-800 fw-semibold d-block fs-7">' + row[3] + '</span>' +
                                        '</div>' +
                                        '</td>' +
                                        '<td>' +
                                        '<div class="text-center pe-0">' +
                                        '<span class="text-gray-800 fw-semibold d-block fs-7">' + row[4] + '</span>' +
                                        '</div>' +
                                        '</td>' +
                                        '<td>' +
                                        '<div class="text-center pe-0">' +
                                        '<span class="text-gray-800 fw-semibold d-block fs-7">' + row[5] + '</span>' +
                                        '</div>' +
                                        '</td>' +
                                        '</tr>';
                            ...
                            if (element.getAttribute("data-kt-daterangepicker-range") === "today") {
                                startDate = moment();
                                endDate = moment();
                            }
...
var KTAppSidebar = function() {
    ...
    return {
        init: function() {
            var i, r;
            n = document.querySelector("#kt_app_sidebar_toggle"),
            e = document.querySelector("#kt_app_sidebar_navs_wrappers"),
            t = document.querySelector("#kt_app_sidebar"),
            n && null !== (i = KTToggle.getInstance(n)) && (i.on("kt.toggle.change", (function() {
