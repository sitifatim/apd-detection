    ...
    init: function() {
        !function() {
            ...
            if (e) {
                ...
                .then(data => {
                    // Menggambar lingkaran dengan data dari server
                    var total = data.on_time + data.late;
                    var onTimePercentage = (data.on_time / total) || 0;
                    var latePercentage = (data.late / total) || 0;

                    // Menggambar lingkaran "on time"
                    i(KTUtil.getCssVariableValue("--bs-success"), t.lineWidth, onTimePercentage);

                    // Menggambar lingkaran "late"
                    i(KTUtil.getCssVariableValue("--bs-danger"), t.lineWidth, latePercentage);
                })
...
var KTChartsWidget5 = function() {
    ...
    , t = function(e) {
        ...
        if (t) {
            fetch('/api/workinghours_monthly')
            .then(response => response.json())
            .then(data => {
                // Mengambil data nama karyawan dan durasi kerja
                var employeeNames = data.name;
                var workDurations = data.duration;

                // Konversi durasi menjadi jam (gunakan fungsi yang telah dijelaskan sebelumnya)
                ...
                var workHours = workDurations.map(convertDurationToHours);
                ...
                var chartConfig = {
                    ...
                    xaxis: {
                        categories: employeeNames,  // Menggunakan nama karyawan sebagai kategori
                        labels: {
                            style: {
                ...
                e.self = new ApexCharts(t, chartConfig);
    ...
    , t = function(e) {
        ...
        if (t) {
            ...
            .then(data => {
                ...
                var workHours = workDurations.map(duration => convertDurationToHours(duration));
